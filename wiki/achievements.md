...
### 배포 구조 — 빌드타임 환경변수 의존 제거
인프라팀의 배포 파이프라인 최적화 요청에서, 환경마다 빌드 산출물이 갈라지는 원인이 컨테이너가 아니라 프레임워크에 있음을 규명했다. Next.js가 공개 환경변수를 빌드 시점에 번들로 인라인하는 구조였다.

서버 렌더 경로에서 빌드 시점에 환경변수를 읽던 페이지를 ISR에서 SSR로 전환하고, 컨테이너 기동 시 주입되는 값을 서버에서 읽는 경로를 추가해 테스트넷·메인넷이 동일 빌드 산출물을 공유하도록 정리.

렌더링 전략을 바꾼 페이지는 1개로 한정했다. 해당 화면은 컴포넌트가 이미 클라이언트 렌더라 서버 작업이 초기 데이터 조회에 국한돼 영향 범위가 작았다. 대신 캐시된 HTML을 공유하던 계층을 잃는 트레이드오프가 있어 PR에 명시했고, 서버 데이터 캐시로 되찾는 안은 당시 운영 규모에서 선제 최적화할 근거가 없어 보류했다.

*기여 범위: 애플리케이션 레이어. 컨테이너 이미지·배포 파이프라인 구성은 인프라팀 담당.*
